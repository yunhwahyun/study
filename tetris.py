...
SPEED_BONUS_MULTIPLIER = 100  # 시간 보너스 배율 (초당)
PENALTY_MULTIPLIER = 50   # 시간 패널티 배율 (초당)

# 레벨별 설정
LEVEL_SETTINGS = {
    1: {"drop_time": 500, "target_time": 60},   # 1레벨: 기본 속도, 목표 60초
    2: {"drop_time": 450, "target_time": 55},
    3: {"drop_time": 400, "target_time": 50},
    4: {"drop_time": 350, "target_time": 45},
    5: {"drop_time": 300, "target_time": 40},
    6: {"drop_time": 250, "target_time": 35},
    7: {"drop_time": 200, "target_time": 30},
    8: {"drop_time": 150, "target_time": 25},
    9: {"drop_time": 100, "target_time": 20},
    10: {"drop_time": 50, "target_time": 15}    # 10레벨: 최고 속도, 목표 15초
}
...
class Tetris:
    ...
    def draw_status(self):
        ...
        if current_time < self.show_bonus_until:
            # 화면 중앙에 크게 표시
            bonus_surface = large_font.render(self.bonus_text, True, self.bonus_color)
    ...
    def calculate_time_bonus(self):
        ...
        if level_time < target_time:
            # 목표 시간보다 빠르게 클리어
            bonus = int((target_time - level_time) * SPEED_BONUS_MULTIPLIER)
            self.score += bonus
            self.bonus_text = f"Speed Bonus: +{bonus}!"
            self.bonus_color = YELLOW
        else:
            # 목표 시간보다 느리게 클리어
            penalty = int((level_time - target_time) * PENALTY_MULTIPLIER)
            self.score = max(0, self.score - penalty)  # 점수가 음수가 되지 않도록
            self.bonus_text = f"Time Penalty: -{penalty}"
